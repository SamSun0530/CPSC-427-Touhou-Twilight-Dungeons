			...
			if (fireRate.is_firing && current_time - fireRate.last_time >= fireRate.fire_rate) {
				...
				for (Entity& player : registry.players.entities) {
					if (entity == player) {
						...
						createBullet(renderer, realmotion.speed_modified, motion.position, mouse_rotation_angle, last_mouse_position - motion.position, true);
					}
					else {
						...
						createBullet(renderer, realmotion.speed_modified, motion.position, enemy_fire_angle, { x, y });
					}
